...
*
*   /// 获取查询详情数据
  fetQueryDetail(type, id, docNum) async {
    await Dialogs.showNormalProgressDialog(context, '加载中，请稍后...', () async {
      try {
        ResultModel resultRegistration = await ArchivesService.getQueryDetail(
            {"type": type, "businessKey": id});
        //兼容老数据
        RegistrationDataMap(resultRegistration);
        re = Registration.fromJson(resultRegistration.data);
        //保存数据
        Provider.of<RegistrationContext>(context, listen: false).setQuery(re);
      } catch (error) {}
    });

    Navigator.push(
        context, MaterialPageRoute(builder: (_) => DetailPage(docNum)));
  }
*
*
*
  ...
  static Future<T> showNormalProgressDialog<T>(BuildContext context,
    ...
    showDialog(
        ...
        builder: (context) {
          _context = context;
          return WillPopScope(
              child: DialogsUtil.normalProgressDialog(message),
              onWillPop: () {
